 ...
 */
public class BasicInfoWSHandler implements WebSocketHandler {

	public static final Map<String, List<WebSocketSession>> basicinfoSocketSessionMap = Maps.newConcurrentMap();
	...
	public void afterConnectionEstablished(WebSocketSession wss) throws Exception {
		...
		List<WebSocketSession> wssLst = basicinfoSocketSessionMap.get(uid);
		if (Util.isEmpty(wssLst)) {
			wssLst = Lists.newArrayList();
			wssLst.add(wss);
			basicinfoSocketSessionMap.put(uid, wssLst);
		} else {
			wssLst.add(wss);
		}
	...
	*/
	public void broadcast(final TextMessage message) throws IOException {
		Set<String> keySet = basicinfoSocketSessionMap.keySet();
		if (!Util.isEmpty(keySet)) {
			Iterator<String> iter = keySet.iterator();
			while (iter.hasNext()) {
				String nextJ = iter.next();
				List<WebSocketSession> lst = basicinfoSocketSessionMap.get(nextJ);
				if (!Util.isEmpty(lst)) {
					//多线程群发  TODO
	...
	public static Map<String, List<WebSocketSession>> getUsersocketsessionmap() {
		return basicinfoSocketSessionMap;
	}
