 ...
 * @Date	 2018年3月29日 	 
 */
public class SimpleSendInfoWSHandler implements WebSocketHandler {

	public static final Map<String, List<WebSocketSession>> simpleSocketSessionMap = Maps.newConcurrentMap();
	...
	public void afterConnectionEstablished(WebSocketSession wss) throws Exception {
		...
		List<WebSocketSession> wssLst = simpleSocketSessionMap.get(uid);
		if (Util.isEmpty(wssLst)) {
			wssLst = Lists.newArrayList();
			wssLst.add(wss);
			simpleSocketSessionMap.put(uid, wssLst);
		} else {
			wssLst.add(wss);
		}
	...
	*/
	public void broadcast(final TextMessage message) throws IOException {
		Set<String> keySet = simpleSocketSessionMap.keySet();
		if (!Util.isEmpty(keySet)) {
			Iterator<String> iter = keySet.iterator();
			while (iter.hasNext()) {
				String nextJ = iter.next();
				List<WebSocketSession> lst = simpleSocketSessionMap.get(nextJ);
				if (!Util.isEmpty(lst)) {
					//多线程群发  TODO
