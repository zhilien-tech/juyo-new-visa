 ...
 */
@IocBean
public class NewLiaoningWandaService extends BaseService<TOrderJpEntity> {
	...
	private ByteArrayOutputStream note(Map<String, Object> tempdata) {
		...
		Date date = new Date();
		if (visaType < 6) {//单次
			map.put("Text15", "PVS" + sdf.format(date));
		} else if (visaType == 14) {//五年
			map.put("Text15", "KSS" + sdf.format(date));
		} else {//三年
			map.put("Text15", "OKS" + sdf.format(date));
		}

		//冲绳
		if (visaType == 2 || visaType == 7) {
			content.append("　　" + companyname).append("根据与").append(dijie).append("的合同约定组织").append(applyinfo.size())
					.append("人赴日本旅游观光，请协助办理赴日签证。 （有料、无料√）").append("\n\n\n")
					.append("冲绳及东北六县的住宿地区（冲绳√，福岛，宫城，岩手，青森，山形，秋田）").append("\n\n").append("东北三县（地区1）的访问地区（福岛，宫城，岩手）");
		}
		//宫城
		else if (visaType == 3 || visaType == 8) {
			content.append("　　" + companyname).append("根据与").append(dijie).append("的合同约定组织").append(applyinfo.size())
					.append("人赴日本旅游观光，请协助办理赴日签证。 （有料、无料√）").append("\n\n\n")
					.append("冲绳及东北六县的住宿地区（冲绳，福岛，宫城√，岩手，青森，山形，秋田）").append("\n\n").append("东北三县（地区1）的访问地区（福岛，宫城√，岩手）");
		}
		//岩手
		else if (visaType == 4 || visaType == 10) {
			content.append("　　" + companyname).append("根据与").append(dijie).append("的合同约定组织").append(applyinfo.size())
					.append("人赴日本旅游观光，请协助办理赴日签证。 （有料、无料√）").append("\n\n\n")
					.append("冲绳及东北六县的住宿地区（冲绳，福岛，宫城，岩手√，青森，山形，秋田）").append("\n\n").append("东北三县（地区1）的访问地区（福岛，宫城，岩手√）");
		}
		//福岛
		else if (visaType == 5 || visaType == 9) {
			content.append("　　" + companyname).append("根据与").append(dijie).append("的合同约定组织").append(applyinfo.size())
					.append("人赴日本旅游观光，请协助办理赴日签证。 （有料、无料√）").append("\n\n\n")
					.append("冲绳及东北六县的住宿地区（冲绳，福岛√，宫城，岩手，青森，山形，秋田）").append("\n\n").append("东北三县（地区1）的访问地区（福岛√，宫城，岩手）");
		}
					...
					map.put("Text3",
							cityName
									...
									+ goFlightNum.substring(goFlightNum.indexOf(" ", goFlightNum.indexOf(" ")) + 1,
											goFlightNum.indexOf(" ", goFlightNum.indexOf(" ") + 1)).replace("//", "/"));
				}
				//天数
		...
		if (!Util.isEmpty(sendVisaDate)) {
			map.put("Text10", dateFormat1.format(sendVisaDate));
			map.put("Text11", dateFormat2.format(sendVisaDate));
		} else {
			map.put("Text10", "");
			map.put("Text11", "");
		}
		...
		if (!Util.isEmpty(outVisaDate)) {
			map.put("Text12", dateFormat2.format(outVisaDate));
		} else {
			map.put("Text12", "");
		}
		//获取模板文件
		URL resource = getClass().getClassLoader().getResource("japanfile/newliaoningwanda/note.pdf");
		TemplateUtil templateUtil = new TemplateUtil();
			...
			if (!Util.isEmpty(ordertripjp)) {
				...
				if (!Util.isEmpty(ordertravelplanList)) {
					...
					for (int i = 0; i < hotelidList.size(); i++) {
						...
						Date addDay = DateUtil.addDay(outDate, querysize - 1);
						...
						//第四列
						if (i == hotelidList.size() - 1) {
							cell = new PdfPCell(new Paragraph(String.valueOf(querysize - 1), font));

						} else {
							cell = new PdfPCell(new Paragraph(String.valueOf(querysize), font));
						...
						//第五列
						cell = new PdfPCell(new Paragraph(room + "SDF", font));
						cell.setHorizontalAlignment(Element.ALIGN_CENTER);
						cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
						cell.setFixedHeight(30);
						maintable.addCell(cell);
	...
	public ByteArrayOutputStream tripInfo(Map<String, Object> tempdata) {
		...
		try {
			...
			if (!Util.isEmpty(sealUrl)) {
				...
				img.setAbsolutePosition(430, height - totalHeight - 230);
	...
	public ByteArrayOutputStream book(Map<String, Object> tempdata) {
		...
		try {
			...
			if (!Util.isEmpty(sealUrl)) {
				...
				img.setAbsolutePosition(430, height - totalHeight - 300);
				img.setAlignment(Paragraph.ALIGN_RIGHT);
