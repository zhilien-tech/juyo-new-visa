...
<body>
			...
			<div class="modal-body" style="height:100%; margin-top:50px;">
				<div class="tab-content">
					...
					<div class="row">
						...
						<c:choose>
							<c:when test="${empty obj.staffvisa.picurl }">
								<img id="visapic" src=" " width="100%">
							</c:when>
							<c:otherwise>
								<img id="visapic" src="${obj.staffvisa.picurl}" width="100%">
							</c:otherwise>
						</c:choose>
						</div>
						<input type="hidden" name="picurl" id="picUrl" value="${obj.staffvisa.picurl}">
					</div>
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								<input name="id" value="${obj.staffvisa.id }" type="hidden"/>
								<label><span>*</span>签证国家：</label>
									<select id="visaCountry" name="visacountry" class="form-control input-sm select2" multiple="multiple">
										<c:if test="${not empty obj.staffvisa.visacountry}">
											<option value="${obj.staffvisa.visacountry}" selected="selected">${obj.staffvisa.visacountry}</option>
										</c:if>
									</select>
							</div>
						</div>
						<div class="col-sm-6">
							<div class="form-group">
								<label><span>*</span>签发编号：</label> <input id="visaNum"
									name="visanum" value="${obj.staffvisa.visanum}" type="text"
						...
						<div class="col-sm-6">
							<div class="form-group">
								<label><span>*</span>签发地：</label> <input id="visaAddress"
									name="visaaddress" value="${obj.staffvisa.visaaddress}" type="text"
									class="form-control input-sm" placeholder=" " />
							</div>
						</div>
							...
							<div class="form-group">
								...
								<select id="visaType" name="visatype"
									...
									<c:forEach var="map" items="${obj.visadatatypeenum}">
										<c:choose>
											<c:when test="${obj.staffvisa.visatype eq map.key}">
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input id="visaDate" 
									name="visadate" value="<fmt:formatDate value="${obj.staffvisa.visadate}" pattern="yyyy-MM-dd" />" type="text"
						...
						<div class="col-sm-6">
							<div class="form-group">
								<label><span>*</span>年限(年)：</label> <input id="visaYears"
									name="visayears" value="${obj.staffvisa.visayears}" type="text"
									class="form-control input-sm autoCalcToDate" placeholder=" " />
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input id="validDate" 
									name="validdate" value="<fmt:formatDate value="${obj.staffvisa.validdate}" pattern="yyyy-MM-dd" />" type="text"
									class="form-control input-sm" placeholder=" " />
							</div>
						</div>
						...
						<div class="col-sm-6">
							<div class="form-group">
								<label><span>*</span>停留时间(天)：</label> <input id="stayDays"
									name="staydays" value="${obj.staffvisa.staydays}" type="text"
									class="form-control input-sm" placeholder=" " />
							</div>
						</div>
					</div>
	...
	<script type="text/javascript">
		...
		function save() {
			...
			if (bootstrapValidator.isValid()) {
				...
				$.ajax({
					...
					data : updateForm,
					dataType:"json",
					url : '${base}/admin/orderUS/updateVisainput.html',
		...
		function closeWindow() {
			window.location.href = '/admin/orderUS/visaInput.html?staffid='+staffid+'&orderid='+orderid;
		}
