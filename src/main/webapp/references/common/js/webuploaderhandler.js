//当为多张图片上传时需要传入name   name为传人input的id, id为 div中的id(后两位00)到uploader数组
    ...
    function inituploader(name, id,  urllist){
        ...
        // 没选择文件之前的内容。
        $placeHolder = $('<div id="dndArea" class="placeholder"><div class="filePicker">点击上传图片</div></div>'),
        
        // 图片容器
        $queue = $('<ul class="filelist"></ul>'),
        $queuelist= $('<div class="queueList"></div>').append($placeHolder).append($queue),
        
        $wrap = $('#uploader_' +id).append($('<p class="title"></p>')).append($queuelist).append($statusBar);
...
//        $progress = $statusBar.find('.progress').hide(),
    ...
    uploader[len] = WebUploader.create({
        ...
        // swf文件路径
        swf: path+'/js/ueditor/third-party/webuploader/Uploader.swf',
        disableGlobalDnd: true,

        paste:'#uploader_'+id+' .queueList',
        chunked: true,
        // server: 'http://webuploader.duapp.com/server/fileupload.php',
        server: BASE_URL + 'ueditor/imageUpload.html?action=uploadimage',
        fileNumLimit: 1,
...
//    });
    ...
    if(urllist.length>0){
         ...
         for(var i in urllist){
        	 //一组图片<input type="hidden" name="picture" value="'+urllist[i]+'">
        	 var $li = $( '<li>' +
                     '<p class="imgWrap"><img src="'+urllist[i]+'"></p>'+
                     '</li>' ),
...
//     });
    ...
    uploader[len].on( 'uploadSuccess', function( file,response ) {
        //图片上传成功的时候设置input表单值,并且删除类型为文件的input项
    	...
    	if(name==""){
    		var input = $("input#webupload_picture") ;
    	}else{
