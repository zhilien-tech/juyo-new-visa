...
SELECT
	...
	oj.orderid,
	o.orderNum,
	( SELECT count( * ) FROM t_applicant_order_jp WHERE orderId = oj.id ) peopleNum,
	o.`status`,
	o.comName,
	o.isDisabled,
	c.source,
...
SELECT
	aoj.orderId,
	a.id,
	GROUP_CONCAT( a.telephone SEPARATOR 'төл' ) applyphone,
	GROUP_CONCAT( CONCAT( a.firstName, a.lastName ) SEPARATOR '、' ) applicants 
FROM
	t_applicant a
	LEFT JOIN t_applicant_order_jp aoj ON aoj.applicantId = a.id
	LEFT JOIN t_order_jp oj ON aoj.orderId = oj.id 
GROUP BY
	aoj.orderId 
